...
const ProductApp = () => {
  ...
  // redirect to product component
   ...
   const handleDelete = async (product) => {
    try {
        const response = await fetch("http://localhost:8080/api/products/delete/${product.id}", {
            ...
            filteredProducts.map((product) => (
              <tr key={product.id} style={styles.tableRow}>
                ...
                <td style={styles.tableCell}>
                  <button style={styles.editButton} onClick={() => handleEdit(products)}>Edit</button>
                  <button style={styles.deleteButton} onClick={() => handleDelete(products)}>Delete</button>
                </td>
              </tr>
            ))
          ) : (
